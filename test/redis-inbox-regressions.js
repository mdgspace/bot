...
test("real Redis atomically accepts duplicates, retains pending work and remembers completion", async t => {
  ...
  await storage.completeEvent(first, "owner-one");
  ...
  await storage.completeEvent(second, "owner-one");
  assert.deepEqual(await storage.pendingEvents(), []);
  await storage.enqueue(doomed);
  const retry = await storage.failEvent(doomed, "still retrying", 2, "owner-one");
  assert.equal(retry.attempts, 1); assert.equal(retry.deadLettered, false); assert(retry.retryAt > Date.now());
  assert.deepEqual(await storage.failEvent(doomed, "missing_scope", 2, "owner-one"), { attempts: 2, deadLettered: true });
  assert.deepEqual(await storage.pendingEvents(), []);
  ...
  await assert.rejects(storage.writeIfLease('{"stale":true}', "owner-one"), /lease is no longer owned/);
  await storage.writeIfLease('{"users":{},"_private":{"keep":true}}', "owner-two");
  ...
  await assert.rejects(storage.failEvent(blocked[0], "temporary", 2, "owner-one"), /lease is no longer owned/);
  await assert.rejects(storage.completeEvent(healthy, "owner-one"), /lease is no longer owned/);
  await storage.failEvent(blocked[0], "temporary", 2, "owner-two");
  ...
  await storage.completeEvent(healthy, "owner-two");
